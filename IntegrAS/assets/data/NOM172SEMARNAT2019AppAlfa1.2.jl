...
# SINAICA's cut thresholds. We suppose these are sensor's cut operation thresholds (?)
PM10thresh = 600;
PM25thresh = 175;
O3thresh = 0.2;
NO2thresh = 0.21;
...
checkMissingValues2
=#
function checkMissingValues2(data, hours, h)
  hr = 0;
  i = 1;
  k = length(hours)
  while i <= k
      ...
      for estacion in red["ESTACIONES"] 
       logEvent("ANALYZING DATA",state["NOMBRE"]*"-"*estacion["NOMBRE"])
        ...
        if length( keys( contaminantes["PM10"] )) > 0       
          ...
          #checkMissingValues2(concentrationPM10, hourPM10, 23)
        ...
        if length( keys( contaminantes["PM2.5"] )) > 0
          ...
          #checkMissingValues2(concentrationPM25, hourPM25, 23)
        ...
        if length( keys( contaminantes["CO"] )) > 0
          ...
          #checkMissingValues2(concentrationCO, hourCO, 23)
        ...
        if length( keys( contaminantes["O3"] )) > 0
          ...
          #checkMissingValues2(concentrationO3, hourO3, 23)
        ...
        if length( keys( contaminantes["SO2"] )) > 0
          ...
          #checkMissingValues2(concentrationSO2, hourSO2, 23)
