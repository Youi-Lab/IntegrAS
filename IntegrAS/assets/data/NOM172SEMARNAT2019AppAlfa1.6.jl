...
numdays = 6;
today = Dates.today();
startDate = string(today-Dates.Day(numdays)); 
curHours = Dates.hour(Dates.now());
timeWindow = 3

# SINAICA's cut thresholds. We suppose these are sensors' cut operation thresholds (?)
PM10thresh = 600;
PM25thresh = 175;
O3thresh = 0.2;
NO2thresh = 0.21
SO2thresh = 0.2;
COthresh = 15;
thresh = Dict("PM10"=>PM10thresh, "PM2.5"=>PM25thresh, "O3"=>O3thresh, "NO2"=>NO2thresh, "SO2"=>SO2thresh, "CO"=>COthresh )
...
function getCriteriaPollutants(stationID)
  ...
  for pollutant in criteriaPollutants
    ...
    dataLine = ""
    try
      response = HTTP.request("POST", url, ["Content-Type" => "application/x-www-form-urlencoded", "charset" => "UTF-8"], HTTP.URIs.escapeuri(params), sslconfig=conf)
...
function retrieveData()
  ...
  try
    ...
    response = HTTP.request("GET", generalDataURL, sslconfig=conf)
    ...
    errorMessage = "Cannot access SINAICA's server"
    logEvent("ERROR", errorMessage)
  end
end
...
function FilterConcentrations(data, ind)
  for i = 1: length(data)
    if data[i] < 0
      data[i] = 0;
    elseif data[i] > thresh[ind]
      data[i] = 0;
    end  
  end
end
...
checkMissingValues
=#
function checkMissingValues(datesStrings, data, hours, ndays)

  nhours = ndays*24; # Total number of hours
  houR = zeros(Int64, nhours)

  for i = 1:24:nhours-24+1
    for j = 0 : 23
      houR[i+j]= j;
    end  
  end  
  datA = zeros(Float64, nhours)

  dayid = Int64[] 
  for s in datesStrings
    y = Dates.Day(today-Dates.Date(s));
    push!(dayid, y.value + 1);
  end

  for i = 1 : length(dayid)
    index = (ndays-dayid[i])*24+1+hours[i]
    houR[index] = hours[i]
    datA[index] = data[i]
  end

  totH = (ndays-1)*24 + curHours;
  return houR[1:totH], datA[1:totH]
...
function CPM(concentraciones, horas) 
  ...
  lv = length(vals)
  if concentraciones[lv] <= 0
    return 0 # Data not available for the current hour
  end
  if lv < horas
       ...
       logEvent("ANALYZING DATA",state["NOMBRE"]*"-"*estacion["NOMBRE"])
        ...
        if length( keys( contaminantes["PM10"] )) > 0       
          for valinfo in contaminantes["PM10"]
            ...
            push!(datePM10, string(valinfo["fecha"]) )
          end
          
          FilterConcentrations(concentrationPM10, "PM10")

          houR, datA = checkMissingValues(datePM10, concentrationPM10, hourPM10, numdays+1)
          concentrationPM10 = datA;
          hourPM10 = houR;
        ...
        if length( keys( contaminantes["PM2.5"] )) > 0
          for valinfo in contaminantes["PM2.5"]
            ...
            push!(datePM25, string(valinfo["fecha"]))
          end
          FilterConcentrations(concentrationPM25, "PM2.5")

          houR, datA = checkMissingValues(datePM25, concentrationPM25, hourPM25, numdays+1)
          concentrationPM25 = datA;
          hourPM25 = houR;
        ...
        if length( keys( contaminantes["CO"] )) > 0
          for valinfo in contaminantes["CO"]
            ...
            push!(dateCO, string(valinfo["fecha"]))
          end
          #println(length(hourCO), length(concentrationCO))
          FilterConcentrations(concentrationCO, "CO")

          houR, datA = checkMissingValues(dateCO, concentrationCO, hourCO, numdays+1)
          concentrationCO = datA;
          hourCO = houR;
        ...
        if length( keys( contaminantes["NO2"] )) > 0
          for valinfo in contaminantes["NO2"]
            ...
            push!(dateNO2, string(valinfo["fecha"]))
          end
          #println(length(hourNO2), length(concentrationNO2))
          FilterConcentrations(concentrationNO2, "NO2")

          houR, datA = checkMissingValues(dateNO2, concentrationNO2, hourNO2, numdays+1)
          concentrationNO2 = datA;
          hourNO2 = houR
        ...
        if length( keys( contaminantes["O3"] )) > 0
          for valinfo in contaminantes["O3"]
            ...
            push!(dateO3, string(valinfo["fecha"]))
          end

          FilterConcentrations(concentrationO3,"O3")

          houR, datA = checkMissingValues(dateO3, concentrationO3, hourO3, numdays+1)
          concentrationO3 = datA;
          hourO3 = houR
        ...
        if length( keys( contaminantes["SO2"] )) > 0
          for valinfo in contaminantes["SO2"]
            ...
            push!(dateSO2, string(valinfo["fecha"]))
          end
          FilterConcentrations(concentrationSO2, "SO2")

          houR, datA = checkMissingValues(dateSO2, concentrationSO2, hourSO2, numdays+1)
          concentrationSO2 = datA;
          hourSO2 = houR
...
function dumpSummary()
  ...
  for station in stations
    ...
    file = open("dump/dump"*station["IDSTATION"]*".txt", "w")
    println(file,"***************************************************************")
...
while(true)
  ...
  logEvent("PROCESSING", "Waiting for new data.")  
	t = Timer(1200,interval=0) #= fourty-minute delay =#
	wait(t)
end
